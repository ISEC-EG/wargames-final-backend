...
describe('GET team profile GET /api/team/  && GET /api/team/:id', () => {
    ...
    describe('GET profile without headers (token)', () => {
        it('Without headers token', () => {
            return request.get('/api/team/').send().then(response => {
    ...
    describe('GET profile with invalid headers (token)', () => {
    ...
    describe('GET my profile successfully', () => {
        it('my profile', () => {
            return request
                .get('/api/team/')
    ...
    describe('GET User profile successfully', () => {
        it('user profile not found', () => {
            return request
                .get(`/api/team/${mongoose.Types.ObjectId()}`)
                .set({ authorization: tokenUser })
                .send({ name: teamNameUser })
                .then(response => {
                    expect(response.status).to.equal(409);
                    expect(response.body).to.have.property('message').to.equal("Team not found");
                });
        });

        it('user profile Invalid id', () => {
            return request
                .get(`/api/team/${faker.name.firstName()}`)
                .set({ authorization: tokenUser })
                .send({ name: teamNameUser })
                .then(response => {
                    expect(response.status).to.equal(400);
                    expect(response.body).to.have.property('message').to.equal("Invalid Team");
                });
        });

        it('User profile', () => {
            return request
                .get('/api/team/60d07d4797113536cc9b10be')
                .set({ authorization: tokenUser })
                .send(teamName)
                .then(response => {
                    expect(response.status).to.equal(200);
